...
def handle_client(client_socket, client):
    """
    client_socket - what is out from proxy server
    client - what receives from client
    """
    with client:
        while True:
...
def process_connection(server_socket, client_socket):
    """
    server_socket - what is out from proxy server
    client_socket - what receives from client to proxy in the server
    """
    while is_running:
...
def main():
    ...
    tunnels = []
    while not finished:
        command = input('connect <source port> <destination port> ->')
